...
export const createJuntasReceptoras = (set) => ({
    ...
    getTodasJuntas: async () => {
        try {
        const response = await axios.get('http://52.200.0.69:5000/api/v1/central_electorals/')
    ...
    getJunta: async (id) => {
        try {
        const response = await axios.get(`http://52.200.0.69:5000/api/v1/central_electorals/${id}`)
